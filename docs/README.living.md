...
> **#151 — integration tests share the dev/demo database (no isolated test DB); side-effecting tests must be mocked or scoped; full test-DB isolation is a deferred task.**
> Surfaced while building notification slice 2: `tests/setup.ts` loads `.env.local`, so every integration test runs against the same Postgres as `npm run dev` — the database holding the live demo content. `vitest.config.ts` documents this as intentional ("Integration tests share one real database", `maxWorkers: 1`). Two consequences were found and one was actively harmful:
> **(a)** `lib/db/seeds/__tests__/demo.seed.integration.test.ts` shells out to the real `seed:demo` (`env: { ...process.env }`), whose contract is "wipe all `isDemo=true` rows and rebuild" (#139). So `npm test` regenerates your demo data from scratch — this is why demo content "drifts" after a test run. Not corruption; re-seeding.
> **(b)** The slice-2 cron auth test exercised the 200 path through the real route handler, which ran the full `deliverDueReminders()` pipeline against the live DB and the real wall-clock date, writing un-cleaned ledger rows for real users. **Fixed immediately:** the cron auth test moved to `app/api/cron/notifications/__tests__/route.test.ts` with `deliverDueReminders` mocked — it now asserts the authorization gate (401/401/401-fails-closed/200 + pipeline invoked/not), never touching the DB. Logged as lesson 0008.
> **Prod safety:** today nothing points tests at prod — there is no CI (empty `.github/workflows`), and prod `DATABASE_URL` lives in Vercel/Neon env vars, not `.env.local`. **But there is no guardrail:** `demo.seed.ts` has no environment check (no `NODE_ENV`/host guard) and will `DELETE … WHERE isDemo=true` against whatever `DATABASE_URL` it is handed. The only thing protecting prod is that the URL happens to point at localhost.
> **Deferred proper fix (own task, `.claude/instructions/test-db-isolation.md`):** a dedicated test database (`.env.test` → `utility_bills_crm_test`, migrated separately, `tests/setup.ts` switched to it) plus a fail-closed environment guard on the demo seed (refuse to run against a non-local/production host). Until then: when iterating, run targeted test files, not the full suite.

> **#152 — Telegram notifications slice 3 (per-user linking): two lifetime-separated tables, fail-closed webhook, per-user delivery channel.** Realizes the linking stage of #150. A bot cannot message a user first, so linking is user-initiated: Settings issues a one-time token and a `t.me/<bot>?start=<token>` deep-link; the user presses Start; Telegram POSTs `/start <token>` to a public webhook that binds the sender's `chat_id` and consumes the token. Slice 4 (reminder UI + gating) is separate.
> **Schema — two tables, separated by lifetime.** `telegram_channels` (durable, 1:1 with the user via UNIQUE `user_id`, holds `chat_id` + display `label` + `linked_at`) is what delivery resolves against; `telegram_link_tokens` (transient, prunable, one active per user — issuing a new one deletes the prior unconsumed token, single-use via `consumed_at`) carries the deep-link binding. Both **hard-delete** (operational/service data, no audit value — the `sessions` precedent, not the soft-delete norm).
> **`chat_id` is Postgres `bigint` read as a JS `number`** (Drizzle `mode: "number"`): Telegram ids exceed int32 (so not `integer`) but are guaranteed ≤ 52 bits, and a number serializes into the `sendMessage` JSON body where a JS `BigInt` would throw. (Distinct from `auth.expires_at`, which stays `integer` only because the adapter requires it.)
> **Webhook is fail-closed** on Telegram's `X-Telegram-Bot-Api-Secret-Token` header vs `TELEGRAM_WEBHOOK_SECRET`; a missing secret env rejects every request (and warns once). Any other update — non-`/start`, or a token that is unknown / expired / already consumed — is acknowledged with 200 and binds nothing, so Telegram does not retry. The side-effecting POST is the inbound-webhook contract (it is secret-gated and unlinked), a scoped analogue to the cron GET exception (#slice-2). Webhook registration is a one-time `npm run telegram:register-webhook` setup step (setWebhook with URL + secret), not runtime.
...
> **#156 — Metered tariff zones unified on one canonical vocabulary; rate-zone count derived from the meter; zone contiguity enforced at the DB.** The same "metered tariff zones" concept rendered across ~16 surfaces with divergent labels, units, and conflicting semantics (3-zone `Peak/Shoulder/Off-peak` in some places vs `Day/Night/Peak` in others), so input forms contradicted read surfaces for the same meter; separately, update-contract hard-coded the rate-field count as `supportsZones ? 3 : 1`, making 2 zones unreachable and rendering a phantom T3 for a 2-zone meter. Four locked decisions:
> **(1) One canonical, count-aware vocabulary in a single cross-cutting source.** `lib/constants/zones.ts` owns the key maps and derivation (`ZONE_LABEL_KEYS`/`ZONE_SUMMARY_KEYS`/`ZONE_SHORT_TAGS`, `tariffZoneCount`, `zoneLabelKeys`/`zoneSummaryKey`); a new top-level `zones` i18n namespace holds the label + summary strings and a new `tariffs` namespace holds the rate vocabulary (`perUnit`/`perMonth`/`fixed`, lifted out of `services.detail.contract.rates`, which is deleted). All ~7 diverged label/summary key sets and the hardcoded `T1:/T2:` literals were removed; the canon is 1→`Single rate`, 2→`Day`/`Night`, 3→`Peak`/`Shoulder`/`Off-peak`, with a count-independent short `T1/T2/T3` for chart legends and compact cells. **Vocabulary lives in code, not a tracker doc** (consistent with the `forms-system.md` removal). A ru mistranslation was corrected in passing (`t2Shoulder` "Плечо" → "Полупик").
> **(2) One shared pure/controlled input** `features/tariffs/components/tariff-rate-inputs` (tariffs domain), used by both the add-service and update-contract forms; deep-imported by those client forms (the tariffs barrel re-exports server-heavy `lib.ts`, lesson 0009). `FormTextField` gained a backward-compatible `inputStyle` prop for the per-zone tint rather than a fork.
> **(3) Zone-count rule** (`features/tariffs/rate-zone-count.ts`, `rateZoneCountFor`): rate-zone count = the service's meter zone count when metered + `supportsZones` + a meter exists, else 1 — replacing the `supportsZones?3:1` hardcode and fixing update-contract (2 zones reachable, no phantom T3). Both update-contract entry points (the intercepting `@modal` route and the full-page fallback) fetch the meter and apply the rule. **Count source stays context-local:** tariff-rate reads (chips, contract-history timeline) keep counting per-era from the tariff's own non-null rates — deriving labels from an ambient/current meter count was rejected because it would mislabel historical tariffs across meter replacements.
