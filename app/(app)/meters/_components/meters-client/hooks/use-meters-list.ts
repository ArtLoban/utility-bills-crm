...
import type { TFilterState, TMeterStatus } from "../../../_data/mock";
...
export const useMetersList = (initialMeters: TMeterGlobalRow[]) => {
  ...
  const filteredMeters = useMemo(() => {
    let rows = [...initialMeters];

    if (filters.status === "active") rows = rows.filter((r) => r.meter.validTo === null);
    else if (filters.status === "historical") rows = rows.filter((r) => r.meter.validTo !== null);

    if (filters.property !== "all") rows = rows.filter((r) => r.property.id === filters.property);
    if (filters.service !== "all")
      rows = rows.filter((r) => r.serviceType.code === filters.service);

    // Already sorted server-side by property.name, sortOrder, validFrom DESC.
    return rows;
  }, [initialMeters, filters.property, filters.service, filters.status]);
  ...
  const propertyCount = useMemo(
    () => new Set(filteredMeters.map((r) => r.property.id)).size,
    [filteredMeters],
  );

  const activeCount = useMemo(
    () => filteredMeters.filter((r) => r.meter.validTo === null).length,
    [filteredMeters],
  );
