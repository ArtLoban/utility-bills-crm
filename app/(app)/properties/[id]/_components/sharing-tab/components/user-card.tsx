    ...
    <div className="flex flex-row gap-[14px] rounded-lg border border-zinc-200 bg-white px-5 py-4 shadow-[0_1px_2px_rgba(24,24,27,0.05)]">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-row flex-wrap items-center gap-[6px]">
          <span className="text-[14px] font-semibold tracking-[-0.1px]">{user.name}</span>
          {user.isYou && (
            <span className="rounded border border-zinc-200 bg-zinc-100 px-[6px] py-[1px] text-[11.5px] font-medium text-zinc-500">
              You
            </span>
          )}
          {showInlineRole && <RoleBadge role={user.role} />}
        </div>
        <div className="mt-[2px] text-[12.5px] text-zinc-500">{user.email}</div>
        <div className="mt-1 text-[12px] text-zinc-500">{user.meta}</div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-[6px]">
        ...
        {showLeave && (
          <button
            onClick={onLeave}
            className="h-[30px] cursor-pointer rounded-[6px] border border-zinc-200 bg-transparent px-3 text-[13px] font-medium text-[#dc2626]"
          >
            Leave property
          </button>
        )}
