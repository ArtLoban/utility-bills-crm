...
import { ROUTES, paymentPath, serviceDetailPath } from "@/lib/routes";
...
export const PaymentDetailActions = ({ payment, canMutate, serviceLabel }: TProps) => {
  const { payment: record, property } = payment;
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await softDeletePayment(record.id);
      if (!result.ok) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-2">
      ...
      {canMutate && (
        <>
          <LinkButton
            href={`${paymentPath(record.id)}/edit`}
            icon={Pencil}
            text={tActions("edit")}
            variant="default"
            size="default"
