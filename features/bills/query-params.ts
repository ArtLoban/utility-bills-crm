...
import type { TBillsListParams, TBillSortColumn } from "./types";
...
const parseSortBy = (raw: string | undefined): TBillSortColumn => {
  const cols: readonly string[] = BILLS_SORT_COLUMNS;
  return cols.includes(raw ?? "") ? (raw as TBillSortColumn) : "periodMonth";
};
...
// Normalises raw URL search params into a validated TBillsListParams.
// All invalid values are silently replaced with their defaults.
// Period preset resolution (last12 → dateFrom/dateTo) happens in page.tsx
// before this function is called — only concrete dates arrive here.
export const parseBillsParams = (
  ...
  const rawPageSize = str("pageSize");
  const rawSortBy = str("sortBy");
  const rawSortOrder = str("sortOrder");

  const page = parseIntGte1(rawPage, 1);
  const rawSize = rawPageSize !== undefined ? parseInt(rawPageSize, 10) : NaN;
  const pageSize =
    Number.isFinite(rawSize) && rawSize >= 1 ? Math.min(rawSize, PAGE_SIZE_MAX) : PAGE_SIZE_DEFAULT;

  const result: TBillsListParams = {
    page,
    pageSize,
